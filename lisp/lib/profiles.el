          ...
          (with-file! init-file
            (let ((regen-message
                   (if (doom-guix-managed-p)
                       "The installed version of Doom has changed since the last Guix reconfigure.\n\nRun 'guix home reconfigure' to fix this."
              ...
              (prin1 `(or (equal doom-version ,doom-version)
                          (error ,regen-message ,doom-version doom-version))
                     (current-buffer)))
...
(defun doom-profile--generate-loaddefs-packages ()
  `((defun doom--startup-loaddefs-packages ()
      ,@(unless (doom-guix-managed-p)
          `((let ((load-in-progress t))
              ,@(doom-autoloads--scan
                 ;; Create a list of packages starting with the Nth-most
                 ;; dependencies by walking the package dependency tree
                 ;; depth-first. This ensures any load-order constraints in
                 ;; package autoloads are always met.
                 (let (packages)
