  ...
  (when (featurep :system 'linux)
    (if (eq (doom-desktop-backend) 'wayland)
        (progn
          ...
          (unless (and (executable-find "grim")
                       (executable-find "slurp"))
            (warn! "Couldn't find grim and slurp. org-download-screenshot will not work.")))
      (progn
        (unless (or (executable-find "maim")
                    (executable-find "scrot")
                    (executable-find "gnome-screenshot"))
          (warn! "Couldn't find maim, scrot or gnome-screenshot. org-download-screenshot will not work."))
